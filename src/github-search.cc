...
#define RAPID_JSON_TYPE(value, name, type, def) value.HasMember(name) ? (value[name].Is##type() ? value[name].Get##type() : def) : def
		...
		std::vector<RepoInfo> GetCandidates(const std::string &name, const std::vector<std::string> &langs) {
			...
			infos = SearchByName(name + ".cget", langs, true);
			if (infos.size())
				return infos;
	  ...
	  std::vector<RepoInfo> SearchByName(const std::string &name, const std::vector<std::string> &ls, bool exactNameMatch) {
			std::vector<RepoInfo> infos;
			...
			for (int forceNameMatch = 1; forceNameMatch >= (exactNameMatch ? 1 : 0); forceNameMatch--) {
